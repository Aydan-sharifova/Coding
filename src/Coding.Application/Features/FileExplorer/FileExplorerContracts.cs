...
public sealed record FileContentDto(Guid NodeId, string Path, string Content, string ContentHash, int VersionNumber, DateTime UpdatedAt);
...
public sealed record SaveFileContentCommand(Guid NodeId, string Content) : IRequest<FileContentDto>;
...
public sealed class SaveFileContentValidator : AbstractValidator<SaveFileContentCommand> { public SaveFileContentValidator() { RuleFor(x => x.NodeId).NotEmpty(); RuleFor(x => x.Content).NotNull(); } }
