...
public sealed class CollaborativeDocumentStore(AppDbContext db) : ICollaborativeDocumentStore
{
    ...
    public async Task<(bool Appended, long SequenceNumber)> AppendUpdateAsync(Guid projectId, Guid fileId, Guid updateId, byte[] update, Guid userId, CancellationToken ct)
    {
        ...
        var sequence = Math.Max(await db.CollaborativeDocumentUpdates.Where(x => x.FileId == fileId).Select(x => (long?)x.SequenceNumber).MaxAsync(ct) ?? 0, await db.CollaborativeDocumentSnapshots.Where(x => x.FileId == fileId).Select(x => (long?)x.SequenceNumber).MaxAsync(ct) ?? 0) + 1;
        db.CollaborativeDocumentUpdates.Add(new CollaborativeDocumentUpdate { ID = Guid.NewGuid(), ProjectId = projectId, FileId = fileId, UpdateId = updateId, EncodedUpdate = update, SequenceNumber = sequence, CreatedAt = DateTime.UtcNow, CreatedByUserId = userId });
        try { await db.SaveChangesAsync(ct); await transaction.CommitAsync(ct); return (true, sequence); }
        catch (DbUpdateException) { await transaction.RollbackAsync(ct); return (false, 0); }
    }
    ...
    public async Task CompactDocumentAsync(Guid projectId, Guid fileId, byte[] state, byte[] vector, long throughSequence, Guid userId, CancellationToken ct) { await using var transaction = await db.Database.BeginTransactionAsync(ct); await SaveSnapshotAsync(projectId, fileId, state, vector, throughSequence, userId, ct); await db.CollaborativeDocumentUpdates.Where(x => x.FileId == fileId && x.SequenceNumber <= throughSequence).ExecuteDeleteAsync(ct); await transaction.CommitAsync(ct); }
}
