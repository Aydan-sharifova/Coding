...
export interface EditorTab { id: string; name: string; path: string; language: EditorLanguage; content: string; savedContent: string; concurrencyToken: string; status: SaveStatus; requestVersion: number; acknowledgedVersion: number; viewState?: editor.ICodeEditorViewState | null; cursor: { lineNumber: number; column: number }; }
...
export const useEditorStore = create<EditorState>()(persist((set, get) => ({
  ...
  openTab: (input) => set((state) => state.tabs[input.id] ? { activeTabId: input.id } : { tabs: { ...state.tabs, [input.id]: { ...input, status: "Saved", requestVersion: 0, acknowledgedVersion: 0, cursor: { lineNumber: 1, column: 1 } } }, openTabIds: [...state.openTabIds, input.id], activeTabId: input.id }),
  activateTab: (id) => { if (get().tabs[id]) set({ activeTabId: id }); },
  updateContent: (id, content) => set((state) => ({ tabs: { ...state.tabs, [id]: { ...state.tabs[id], content, status: content === state.tabs[id].savedContent ? "Saved" : "Unsaved", requestVersion: state.tabs[id].requestVersion + 1 } } })),
  acceptExternal: (id, content, token) => set((state) => ({ tabs: { ...state.tabs, [id]: { ...state.tabs[id], content, savedContent: content, concurrencyToken: token, status: "Saved", requestVersion: state.tabs[id].requestVersion + 1 } } })),
