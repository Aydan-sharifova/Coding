...
class ExplorerStore {
  ...
  load(nodes: WorkspaceNode[]) { this.entities = new Map(nodes.map((node) => [node.id, node])); this.children = new Map(); for (const node of nodes) { const key = node.parentId ?? "root"; this.children.set(key, [...(this.children.get(key) ?? []), node.id]); } this.emit(); }
