...
function Card({ task, onOpen }: { task: ProjectTask; onOpen: () => void }) {
...
function Column({ status, label, tasks, onOpen }: { status: TaskStatus; label: string; tasks: ProjectTask[]; onOpen: (task: ProjectTask) => void }) {
  const drop = useDroppable({ id: `column:${status}`, data: { status } });
  return <section ref={drop.setNodeRef} className={`kanban-column ${drop.isOver ? "over" : ""}`}><header><span className={`status-dot ${status.toLowerCase()}`} /><h2>{label}</h2><b>{tasks.length}</b></header><div className="kanban-list">{tasks.map((task) => <Card key={task.id} task={task} onOpen={() => onOpen(task)} />)}{!tasks.length && <div className="kanban-empty">Drop tasks here</div>}</div></section>;
}
export function KanbanBoard({ tasks, onMove, onOpen }: { tasks: ProjectTask[]; onMove: (taskId: string, status: TaskStatus, previousTaskId?: string, nextTaskId?: string) => void; onOpen: (task: ProjectTask) => void }) {
  const sensors = useSensors(useSensor(PointerSensor, { activationConstraint: { distance: 6 } }));
  const dragEnd = ({ active, over }: DragEndEvent) => {
    if (!over) return; const task = tasks.find((x) => x.id === active.id); if (!task) return;
  ...
  return <DndContext sensors={sensors} onDragEnd={dragEnd}><div className="kanban-board">{columns.map((column) => <Column key={column.status} {...column} tasks={tasks.filter((x) => x.status === column.status).sort((a, b) => a.position - b.position)} onOpen={onOpen} />)}</div></DndContext>;
}
